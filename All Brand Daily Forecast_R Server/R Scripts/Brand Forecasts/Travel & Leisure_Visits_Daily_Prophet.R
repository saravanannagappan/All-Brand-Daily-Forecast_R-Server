...
holiday_names = c('MemorialDay', 'Thanksgiving')
...
# Thanksgiving window is 3 days before the holiday date
len = nrow(holidays[holidays$holiday == 'Thanksgiving',])
holidays[holidays$holiday == 'Thanksgiving', c('lower_window', 'upper_window')] = rep(c(-3, 0), each = len)

# MemorialDay window is the day only
len = nrow(holidays[holidays$holiday == 'MemorialDay',])
holidays[holidays$holiday == 'MemorialDay', c('lower_window', 'upper_window')] = rep(c(0, 0), each = len)
