...
train.raw = train.raw[train.raw$ds>="2018-03-01",]

# View(train.raw)

train.raw[(!is.na(train.raw$y)) & (train.raw$y<4000),'y'] = NA
...
veteransday <- data.frame(
  holiday = 'veteransday',
  ds = dateframe[dateframe$VeteranDay == 1,'date']
  , lower_window = 0
  , upper_window = 0
)
