...
# Remove the data before Jul 2016 as the monthly pattern was completely different from the pattern after that.
train <- train.raw
train <- train[train$ds >= as.Date('2016-07-01'),]
...
holiday_names = c('Thanksgiving', 'Christmas', 'Easter', 'NewYear')
...
# Thanksgiving window is 4 days before the holiday date
len = nrow(holidays[holidays$holiday == 'Thanksgiving',])
holidays[holidays$holiday == 'Thanksgiving', c('lower_window', 'upper_window')] = rep(c(-4, 0), each = len)

# Christmas window is 5 days before the holiday date
len = nrow(holidays[holidays$holiday == 'Christmas',])
holidays[holidays$holiday == 'Christmas', c('lower_window', 'upper_window')] = rep(c(-5, 0), each = len)

# Easter window is 7 days before the holiday date
len = nrow(holidays[holidays$holiday == 'Easter',])
holidays[holidays$holiday == 'Easter', c('lower_window', 'upper_window')] = rep(c(-7, 0), each = len)

# NewYear window is 3 days before the holiday date
len = nrow(holidays[holidays$holiday == 'NewYear',])
holidays[holidays$holiday == 'NewYear', c('lower_window', 'upper_window')] = rep(c(-3, 0), each = len)
